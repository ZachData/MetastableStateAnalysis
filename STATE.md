...
**Last updated:** 2026-09-25 (F1/F12 on the pilot sweep, `status-10.md` §1.13) · **Cap:** 150 lines (tier-0 lint enforces it).
...
| Current stage | Stage 0, option B: all 20 prompts × 19 checkpoints (380 runs) under v2, in 10-h chunks via `tools/run/stage0_chunk.py`, **3 chunks** planned. **Pin `64a4087`** (run tree `../Mets-stage0`). **Chunk 1 DONE** (2026-09-22 21:20 → 2026-09-23 10:31; survived an overnight suspend): 144/380 runs indexed, all populated. **Chunk 2 DONE** (05:35 → 14:50 on 2026-09-24): 377/380. **Chunk 3 DONE** (15:35:56 → 15:58:27, `rc 0, 3/3`). **Stage 0 COMPLETE: 380/380 at pin `64a4087`** (`stage0_chunk plan`: "nothing left"); run tree `../Mets-stage0` removed. Readers select through `data/phase12/stage0_logs/stage0_index.json`. Detail and the guarded launch block: `handoff-10.md` §0.3 (the runbook) |
| Next after it | Stage 1 on the **8 v1 prompts only**. **Step 1, the `ext_sem_threshold` sweep, DONE 2026-09-24** on 136/152 v1 runs through `stage0_index.json` (`status-10.md` §1.6): the stored count is mostly a repeat count (repeats have cosine 1 at layer 0). §1.1's decline is the repeat share falling, and the non-repeat mutual pairs that replace the repeats become similar in the trained embedding, so "lexical → contextual" is backwards on this measure. Re-run at 152/152: unchanged. **Step 2, the token-composition table, DONE 2026-09-24**, re-run at 152/152 v1 runs (`status-10.md` §1.7): clustered vs noise is mostly copy count at init and in shallow layers (HDBSCAN `min_cluster_size=2`), and moderate in the trained model's deep layers. Among unique tokens BPE rank does not predict it; class does, weakly and against trash collection. **Its two parked checks DONE 2026-09-24** (`status-10.md` §1.8): duplicates planted in Gaussian noise reproduce step 0 layer 0's rates, so those are the instrument's; Phase 1's carrying capacity (`max_alive`) is, on these runs, mostly a repeat count at the embedding layer: it peaks at layer 0 in 92 of 133 runs, mean 57–63 at every step against 59.7 repeated token types per prompt. The pilot's own labels give the same 57–63 (§1.11), so where "50–55" came from is still parked. **The co-membership reader DONE 2026-09-24** (`status-10.md` §1.9; `tools/run/p10_comembership.py`): revised after `/challenge-pr` on #92. At init, clusters at depth already follow each token's own random embedding carried in the residual. Training adds class (Δ +0.20 over a random draw, by step 512) and moves unique tokens away from copy groups. Own-embedding similarity adds only +0.12, and adjacency a depth-only share. The first version's "they cluster with tokens the trained embedding calls similar" had no valid step-0 baseline and is withdrawn. **Lexical or not, DONE 2026-09-24** (`status-10.md` §1.10; `tools/run/p10_lexical_carry.py`): revised after `/challenge-pr` on #93. At step 512 the network computes the class grouping (+0.20 beyond own-embedding similarity; a purely lexical kNN cluster scores ≈ 0). Trained, it is mostly what the embedding already groups: at 40 bins, L12 is +0.04 against a +0.04 lexical control, and L24 is +0.05 above its control. That reading depends on controls added post hoc; the pre-stated decile control could not work (the lexical control scores +0.21 under it). Clustered tokens keep less of their individual layer-0 direction. Context vs per-token feature is parked (a context-shuffle test, needs forward passes). **Step 3, the pilot sweep, DONE 2026-09-25** (`status-10.md` §1.11; `tools/run/p10_s1_compare.py`): Stage 0's labels are bit-identical to the WDS backfill's, so the pilot is the only independent sweep. Steps 1 and 2 on it agree with Stage 0 to ≤ 0.004 at all 13 shared steps. §1.1's table was 9 prompts, not 8. The partition moves under ~2e-7 of activation noise, so §3's floor is run to run, and today's bit-exact repeat is only determinism on one machine. **Stage 1 is done by the letter of §1.3.** **§1.9–§1.10 on the pilot DONE 2026-09-25** (`status-10.md` §1.12; `p10_s1_compare.py --per-layer`): the per-layer claims hold on the second partition, with 3 103 / 3 120 and 383 / 384 readings agreeing and every disagreement but one a flip at the ±0.05 floor. **F1/F12's 32–64 window on the pilot DONE 2026-09-25** (`status-10.md` §1.13): holds (per-step means within 0.005 / 0.008, no changed unit flips sign at step 32); below-baseline `Z` lasts past 512 on both sweeps, which §1.4's table had omitted (revised after `/challenge-pr` on #97). **Next:** the shuffle test, matching clusters across checkpoints (handoff Parked; no row follows a cluster between checkpoints), or Stage 2. **The 12 new v2 prompts are held out on 410m** as the confirmation set until registrations are frozen (user, 2026-09-23; `handoff-10.md` §0.4). **Enforced by `core/holdout.py`** (built 2026-09-24): every runner over `data/phase12` refuses them, or is exempt for a reason a test lists. The Phase 10 readers take `--v1-only` or `--allow-holdout` (`status-10.md` §0). Partly seen already via `CLAIM-C` on 1.4b/gpt2-large; scope, count, order and release are open for the user (`docs/PHASE_REVIEW.md` "Open") |
...
**#97** `claude/p10-window-pilot` (§1.13), in `../Mets-work`: open. #96 merged 2026-09-25, its branch and `../Mets-work-2` removed; stale merged remote branches (`s110-review-fixes`, `s19-lexical-vs-context`) deleted. #95 merged 2026-09-25, its branch and `../Mets-work` removed. #94 (#93's `/challenge-pr` answers) merged 2026-09-24, its local branch deleted 2026-09-25. #92 merged 2026-09-24, its branch and worktree removed. #91 merged 2026-09-24, its branch and worktree removed. #90 merged 2026-09-24, its branch and worktree removed. #89 merged 2026-09-24, its branch and worktree removed. #88 merged 2026-09-24, its branch and worktree removed. #87 merged 2026-09-24, its branch and worktree removed. #84 merged 2026-09-24, its branch and worktree removed. #86 merged 2026-09-24, its branch and `../Mets-work-2` removed. #85 merged 2026-09-24, branch deleted. #83 merged 2026-09-24, its branch and worktree removed. #82, #81, #80 merged 2026-09-24.
