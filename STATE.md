...
| Current stage | **First real run done** (smoke, one v1 run, L0/12/18, quick grid; `status-1d.md` "First real run"; output `data/p1d/smoke_2026-09-25/`): ~170 s per layer, outputs populated, 6–7 of 7 families admitted per layer. It found the design question to settle first: ranking by subsample stability drives each family to an extreme scale (centroid families k = 2, agglomerative its finest threshold), so the consensus mixes scales. **Literature scan DONE 2026-09-25** (`p1d_cluster_ensemble/lit-1d.md`): 1d's stage 1 ranks on *raw* stability, which the literature warns trivially favours extreme scales (von Luxburg 2010 §2). The standard remedy (normalising by the null) fails on 1d's data: the null mean is 0 at the fine end, and it moved k-means to k = 4, the other end of the grid (`/challenge-pr` on #99). The scan also covers attention-window communities (the stored `attentions.npz`, as Markov time), wells of the vMF potential `φ_β`, and prompt length before prompt count. **A0 DONE 2026-09-25** (`status-1d.md` "A0"; output `data/p1d/smoke_a0_2026-09-25/`): a partition with > 50 % singletons is now trivial. Agglomerative moves from threshold 0.05 to 0.25 at L12 / L18 but not at L0. The consensus partition hardly moves (ARI ≥ 0.99), but the grading does: L12 core goes from 0 to 257 tokens. The cause is the ensemble's voting (singletons vote against grouping, HDBSCAN's -1 abstains, weights are raw stability), so **core/halo/contested is not a usable readout yet**. L0's pick is token identity (215 clusters = 215 distinct embedding vectors); whether L0 stays in 1d is the user's call. **User's direction (2026-09-25):** the priority is more ways to check that a cluster is real and not an HDBSCAN artefact. D (hierarchy across scales) + C (`δ = cβ^{-1/2}` marked on it): yes; C still waits on β's convention (Blocked 9). **Prompts to maximum length** (Pythia's context is 2048; `wiki_paragraph` is 467 tokens): yes. That needs new text and forward passes, and it breaks comparability with the v1 runs and the holdout, so it is its own unit. **Drift DONE 2026-09-25** (`status-1d.md` "Float-noise drift"; 72 layer-records, 8 v1 prompts × steps 32/512/143000 × L6/12/18, Stage 0 vs pilot; output `data/p1d/drift_2026-09-25/`). Every family picks the same parameters on both sweeps. The consensus moved in 1 of 72 records, shipped HDBSCAN in 13, tuned HDBSCAN in 7 (less often, not less far). The families that never move pick coarse k or, for agglomerative, fine partitions that group identical token strings, so the consensus's stability does not show a reproducible structure. The drift is one prompt, `repeated_tokens`, and so is almost all of Phase 10 §3's floor (correction routed to `status-10.md`). The one consensus collapse needs HDBSCAN's moved labels and two weights together: a knife-edge cut, not a single family's drift. 1d does not reproduce itself bit for bit. **Matched k DONE 2026-09-25, and it found a defect** (`status-1d.md` "Matched k on `repeated_tokens`, and the float32 defect"; `/challenge-pr` on #102 found it, Claude verified): the drift is float32 cancellation in `1 - x·y` (`clustering.py`, 1d's `LayerData`). Refit on float64, shipped HDBSCAN agrees across sweeps at ARI 1 in 8 of 9 `repeated_tokens` records; the stored partitions there are mostly rounding (ARI 0.07–0.66 vs float64 at steps 32/512). `wiki_paragraph`: exact. k-means at fine k is seed-dependent (0.38–0.75 seed 0 vs 1 on `wiki_paragraph`). Routed to `status-10.md`. **Float64 fix DONE 2026-09-26** (`status-1d.md` "Float64 distances, and Phase 10 §3's floor re-run on them"): one distance route, `core.metrics.cosine_distance_matrix`, in `clustering.py`, `LayerData` and the backfill. On float64 the §3 floor goes from ARI p5 0.347 to **1.000** (99.1 % of 2 600 records identical). Stage 0's stored `repeated_tokens` labels are rounding (mean ARI 0.23 to float64); the other 7 prompts' hold. Stored labels are **not** rewritten; re-deriving them and re-running 1d's drift are Parked there. **Merge tree + cross-layer linking DONE 2026-09-26** (`status-1d.md` "Merge tree over scales, and clusters linked across layers"; new sub-experiment F, no tuning, ~1 s per prompt; option D, `lit-1d.md` §5; all 8 v1 prompts, step143000/410m; output `data/p1d/merge_tree_2026-09-26/`). **Revised before merge:** the first version's "splits almost absent, merges at L0→L2" was the method (longest-lived plateau = one blob + outliers; Jaccard cannot see a piece leave a large cluster) and is withdrawn. Now: partition = longest-lived plateau with ≥ 2 clusters of ≥ 4 tokens, links by containment. Findings: the longest-lived scale is one cluster plus outliers in 126 of 192 layer-records, with token 0 (the sink) an outlier of the chosen partition in 131 of 192; the ≥ 2-cluster pick is exactly 2 clusters in 112 of 176 at L3–24, so the scale question is still open. **Null + token 0 DONE 2026-09-26** (`status-1d.md` "The link counts against a size-preserving null…"; independent per-layer relabelling, 2000 draws; output `data/p1d/merge_null_2026-09-26/`): neither depth headline is supported. The L0–L7 merge excess (33 vs 21) is not distinguishable from the null (merge fraction 0.61 real, 0.68 null), and cluster counts fall across L0–L7 (net 100), which alone favours merges. The late tangle share (0.33–0.43) is far *below* the null's 0.96–0.97. Beating this null on persistence is not evidence (neighbouring layers carry over by construction; ~1 stable link per late boundary). Token 0 is not special: without it 194 of 199 partitions are identical and the blob stays in 104 of 192, 41 of them "everything + 1 token": absolute lifetime turns any far token into a blob plateau (revised after `/challenge-pr` on #105). Also fixed: `--subexp F` silently ran the full tuning grid; tied merge heights made unreproducible plateaus. **Gaussian null DONE 2026-09-26** (`status-1d.md` "Matched-covariance Gaussian null"; SigClust's null, `lit-1d.md` §7; raw / centred / centred minus 3 rogue coordinates; 8 v1 prompts × step143000 and step 0 × 25 layers, 200 draws; output `data/p1d/gaussian_null_2026-09-26/`). With every token both checkpoints beat the null and **step 0 beats it harder**: that is token identity (a token's nearest neighbour is the same string for 56–75 % of tokens at step 0). **Deduped** (first occurrence per string), step 0 has nothing beyond Gaussian. Step143000 is lumpier **only locally**: closer nearest neighbours in 168 of 168 records, and HDBSCAN finds 20 groups vs the Gaussian's 6 (centred), rising with depth. **Nothing global:** 2-means and the merge tree's 2-cluster pick and its lifetime are what the Gaussian gives. The null is not at nominal level (`ci2` false alarms 6–25 %), so each z is read against a calibration run. **Next (user's order, 2026-09-26):** (3) attention communities from `attentions.npz` as an independent check (`lit-1d.md`); what "the same null" means there is open (Parked 10 in `status-1d.md`: a kernel on the draws needs β, Blocked 9). Then long prompts. The weighting decision and `design-1d.md` after. Beside it: marking C's `δ = cβ^{-1/2}` on the merge tree (Parked in `status-1d.md`, waits on β's convention, Blocked 9); the theory's own definitions (fixed-scale F13/F14, `p10_cluster_function/math-10.md` §7; persistence across layers) |
