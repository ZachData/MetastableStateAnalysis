...
| 2 | Extend `CLAIM-C`'s homogeneity calibration to 20 prompts and rescore the v2 arms | `CLAIM-C` | free, ~45 min CPU (§3.46) | order vs the holdout (`docs/PHASE_REVIEW.md` "Open" 3) |
...
permutation null needs ≥ 1 599 draws, and an exact sign-flip needs ≥ 11 informative
units (`tools/math_checks/evalue_unit_count.py`). A claim's E is the product over its
adjudicated predictions (`core/evalues.py` `combine`), so a row below 20 still counts;
the last column says whether it can decide its claim **alone**. The calibration records'
...
|---|---|---|---|---|
| `CLAIM-C` | prompt (sign pattern, homogeneity-corrected) | 8 prompts: raw 0.0078, corrected 0.0661 → **E 1.94**. 12: raw 0.00049 → E ≤ 22.6, corrected untabulated. 20: 5 000 null patterns → E ≤ 35.4, corrected untabulated | at 12 only if the correction costs < 20 %; at 20 plausibly | `claims/calibration/claim_c_homogeneity.json`, `claims/audits/claim_c_real_run.json` |
| `P-AB1` | prompt (sign over ablation points; odd counts only) | 2⁻ⁿ over informative prompts: 8 → E 8.0; 12 → E 32; the calibrated design has 6 prompts | at ≥ 11 informative prompts | `claims/calibration/patching_exponent.json` |
| `P-ST1` | matched-norm vector pair | ≈ 2⁻ᵏ over informative pairs, independent of total pairs; 8 informative → E 8.0 | at ≥ 11 informative pairs | `claims/calibration/steering_sign.json` |
...
What this changes: `CLAIM-C` at 8 prompts cannot matter (E ≤ 1.94), so its rescore
is worth doing only at 12 or 20 prompts, and those are the held-out prompts (§3.3).
...
| `P6-R4`'s unit `model` on Pythia (Parked 6) | `p6_subspace/r2_r4_null.py` `_draw_rngs`: one seed per draw, a fresh generator per layer; each layer draws `random_orthogonal_subspace_pair(d_model, dim U_neg, dim U_A, rng)` | yes, narrowed | Where two layers' subspace dimensions match, they get the same subspace (the tied reading). Where they differ, the same seed draws a different-shaped matrix, so the subspaces are effectively independent. On Pythia "model" is therefore "same subspace where the shapes agree", which is still conservative (the recorded table keeps `model` at nominal at ρ = 0). Recommend: keep `model`, amend the notes to say this |

### 3.3 Phase 10's route into the registry

1. **Order.** `CLAIM-C` only matters at 12 or 20 prompts (§3.1), and those are the
   confirmation set. Rescoring it on 410m v2 runs before Phase 10 registers would spend
   the 12. Recommend: freeze Phase 10's registrations first, then rescore `CLAIM-C` and
   score Phase 10's rows on the same release. The decision is still the user's ("Open" 3).
...
3. **Unit and floor before registering.** Say whether F14's unit is the prompt; if so it
   needs ≥ 11 informative prompts of the 12 to decide alone. That margin is one prompt,
   so a per-prompt sign design is fragile; a permutation design with ≥ 1 600 draws over
   a finer unit is not. Put `max_attainable_average_E` on the row's face, as the 10 A0
   runners do.
