...
| ~~6~~ | **2411.04990** *Clustering in Causal Attention Masking* | **READ 2026-09-20 — `[R]`, see `p10_cluster_function/lit-10.md` §11.** The parking correspondence is exact and the count law is distribution-free (`math-10.md` §7.2); **2d's gradient-flow framing is narrowed, not void** — the masked dynamics is a *sequential* gradient flow with a per-particle energy, so Łojasiewicz fails but the curvature claim survives per token; and §B.3 measures `albert-xlarge-v2`'s `V`-spectra, which is prior art for this project's V-attractive/V-repulsive split |
