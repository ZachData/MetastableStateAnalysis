...
| 7 | Card: 9, in a new `p9_metric_intervention/status-9.md` (card, `plan-9.md` §8's ladder state, corrections received), depending on 1c, 2, 7d, 7e, 8, 10 and feeding 10; those five gained 9 in Feeds (1c's "no status file" note dropped). A status file makes a phase: `tools/render_experiments.py` needed a no-predictions entry for 9. Nothing has run under Phase 9's name and nothing on disk is its. Found: 3 of the ladder's 14 rows were stale when written (E0 ran as Phase 10's F1; E5's unit was decided 2026-08-25; §11's "nobody noticed" 5c's LM head, recorded in July; `LESSONS.md` lesson 1), and `notes-9.md` §7 and `plan-9.md` §4.7 still called the γ-patch read-side after §4.1's correction. Pointers added in place; 7 corrections routed. Live 6's card calls the inversion "unresolved" where frozen 6's reads it chance-normalised (E4, in 9's Open threads) | **done** 2026-09-24 |
| 8 | Card: 10 | open |
