...
# sklearn (and hdbscan when installed): the heavy tier, no model, no run dirs.
...
class TestConstants:
    ...
    def test_shipped_params_are_the_ones_phase1_actually_calls(self):
        # clustering.py's call is hdbscan.HDBSCAN(**params) with
        # params = {"min_cluster_size": 2, "metric": "precomputed"} —
        # everything else library default. (Until 2026-09 the kwargs were
        # inline; the dict exists so the run records them.) If that call
        # changes, P-C2 is comparing against a partition nobody ships.
        import ast
        ...
        assert len(calls) == 1, "expected one HDBSCAN(...) call in p1_mstate_tracking/clustering.py"
        (call,) = calls
        if call.keywords and call.keywords[0].arg is None:      # HDBSCAN(**name)
            name = call.keywords[0].value.id
            dicts = [n.value for n in ast.walk(tree)
                     if isinstance(n, ast.Assign) and isinstance(n.value, ast.Dict)
                     and any(isinstance(t, ast.Name) and t.id == name for t in n.targets)]
            assert len(dicts) == 1, f"cannot find the single {name} = {{...}} literal"
            kwargs = ast.literal_eval(dicts[0])
        else:
...
class TestStability:
    ...
    def test_separation_is_nan_when_every_token_is_its_own_cluster(self, planted):
        # What a fine grid point made of a null draw on the first real run
        # (467 labels, 467 tokens): sklearn's silhouette raises there.
        assert np.isnan(separation_score(np.arange(planted.n), planted))
        labels = np.arange(planted.n)
        labels[:3] = -1                       # refusals do not rescue it
        assert np.isnan(separation_score(labels, planted))
