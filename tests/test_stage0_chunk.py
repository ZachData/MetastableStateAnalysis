...
def _run(root, step, prompt, *, sha=PIN, battery=s0.BATTERY_HASH_V2,
         labels=None, pair=None, manifest=True, exp="exp", n_tokens=404, wall=200.0):
    exp_dir = root / exp
...
def test_fit_scale_keeps_probe_until_enough_runs():
    few = [s0.Done(0, "p", None, 999.0, 404)] * (s0.MIN_REFIT - 1)
    assert s0.fit_scale(few) == s0.PROBE_SECONDS
    many = [s0.Done(0, "p", None, 100.0, 404)] * s0.MIN_REFIT
...
def test_plan_never_exceeds_budget_and_splits_a_checkpoint():
    ...
    n = {p: 404 for p in prompts}
    per = s0.estimate_seconds(404, s0.PROBE_SECONDS)
    budget = s0.LOAD_SECONDS * 2 + per * 25   # one checkpoint and 5 of the next
    rem = [(s, list(prompts)) for s in s0.CHECKPOINTS]
    chunk = s0.plan_chunk(rem, n, s0.PROBE_SECONDS, budget)
    assert [len(p) for _, p in chunk] == [20, 5]
    assert chunk[1][1] == prompts[:5]
