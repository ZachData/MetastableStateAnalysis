...
extract_qk_per_head       : per-head (d_model, d_head) W_Q / W_K matrices
...
# ---------------------------------------------------------------------------
# QK per-head matrices  (Bug 2 fix)
# ---------------------------------------------------------------------------

def extract_qk_per_head(model, model_type: str | None = None) -> dict:
    """
    Per-head W_Q, W_K matrices in canonical (d_model, d_head) orientation.

    The orientation is chosen so that, for residual-stream activations
    x_i, x_j ∈ R^{d_model} (column vectors), per-head Q/K projections are
        q_i = WQ_h^T @ x_i      (shape d_head)
        k_j = WK_h^T @ x_j      (shape d_head)
    and the attention bilinear is
        q_i · k_j = x_i^T (WQ_h @ WK_h^T) x_j = x_i^T M x_j,  M ∈ R^{d×d}.
    `decompose_qk_matrix` consumes this M.  Always orient WQ/WK consistently
    here; never push the orientation question downstream.

    For nn.Linear weights stored as (d_total, d_model), the per-head slice is
    (d_head, d_model) and we transpose.  For Conv1D weights stored as
    (d_model, d_total), the per-head slice is already (d_model, d_head).

    Returns
    -------
    {
      "wq_per_head":  list[(d_model, d_head)] | list[list[...]],
      "wk_per_head":  list[(d_model, d_head)] | list[list[...]],
      "is_per_layer": bool,
    ...
    if model_type == "albert":
        ...
        wq_per_head, wk_per_head = [], []
        for h in range(n_heads):
            s, e = h * d_head, (h + 1) * d_head
            wq_per_head.append(np.ascontiguousarray(WQ[s:e, :].T))   # (d_model, d_head)
            wk_per_head.append(np.ascontiguousarray(WK[s:e, :].T))
        return {
    ...
    raise NotImplementedError(f"extract_qk_per_head: unsupported model_type={model_type}")


def _add_qk_arrays_to_decomposition(
    arrays:  dict,
    qk_data: dict,
) -> None:
    """
    Mutate `arrays` in place to add wq_head{h}_{suffix} / wk_head{h}_{suffix}
    keys.  The suffix matches the OV save convention: "shared" for ALBERT,
    "layer_{i}" for per-layer models.  Picked up by run_6._load_ov_weights.
    """
...
def save_weight_decomposition(
    ...
    Writes:
      ov_weights_{model}.npz    — OV matrices (total + per-head) and raw
                                  W_Q / W_K per head (Bug 2 fix)
    ...
    ----------
    model, model_type
        Required to extract raw W_Q / W_K per head.  If model is None the
        QK weight arrays are omitted (legacy behaviour, not recommended).
    """
    ...
    # Bug 2 fix: write raw W_Q / W_K per head so qk_decompose has non-empty
    # qk_matrices and the P6-I2 pipeline runs over actual heads.
    if model is not None:
        per_head_qk = extract_qk_per_head(model, model_type=model_type)
        _add_qk_arrays_to_decomposition(ov_arrays, per_head_qk)
...
def analyze_weights(model, model_name: str, save_dir: Path) -> dict:
    ...
    5. Save everything (including raw W_Q / W_K per head)
    ...
    save_weight_decomposition(
        ov_data, decomps, projectors, qk_data,
        save_dir, model_name,
        model=model,          # pass model so W_Q/W_K are written
        model_type=None,      # _detect_model_type resolves from model_name
    )
