...
- **Results:**
  ...
  - At step 0 layer 0 the partition is what HDBSCAN makes of Gaussian noise with planted duplicates (twins noise at 0.14, singletons clustered at 0.43). At layer 0 the cluster count equals the prompt's repeated token types at every checkpoint, and ≥ 63 % of clusters at any layer hold a repeat; `repeated_tokens` reaches ~50 deep-layer clusters without repeats. No null — `p10_cluster_function/status-10.md` §1.8
  - Lemma C.1's saturation is a formula for Phase 1's 50–55 carrying capacity; a repeat-structure count competes with it — `p10_cluster_function/math-10.md` §5.4, §3.53, `p10_cluster_function/status-10.md` §1.8
- **Superseded / wrong:**
...
- **Depends on:** 1@54c1f37216, 5c@e75b33ae46, 7d@19b7d835b7, 7e@0c1071db50, 8@8cc3fb223c, 9@e70efd632b
- **Feeds:** 9
- **Open threads:**
  ...
  - Is Phase 1's carrying capacity a repeat count? `max_alive` against repeated types per prompt, and what sets `repeated_tokens`' ~50 deep-layer clusters — `p10_cluster_function/status-10.md` §1.8
...
- **Reviewed:** 2026-09-24 · body `4e1cd4dae2`
<!-- /phase-card -->
...
### 1.8 Two checks on §1.7 — **at step 0 layer 0 the partition is what HDBSCAN makes of noise with duplicates; cluster count ≈ repeated types at layer 0 at every checkpoint**
...
- **At layer 0 the cluster count is a count of repeated types at every
  checkpoint** (ratio 0.92–1.01 over all 19 steps). At every layer and step,
  ≥ 63 % of clusters hold a repeat (minimum: step 143000, L19).
...
- **`repeated_tokens` is the exception, and it cuts the other way.** Its text
  has 1 repeated token type; HDBSCAN finds 2 clusters at L0 but 52 at L12 and
  55 at L24 at step 0 (43 / 42 at step 143000). So ~50 deep-layer clusters
  arise without repeat structure, from something else (position is the first
  suspect), and pure noise does not produce them (arm 2 of (a)).
- **What this does to Phase 1's carrying capacity: a competing explanation,
  not a replacement.** `max_alive` (`cluster_tracking.py`) is computed from
  these same labels, and a prompt's repeated-type count does not change with
  training. So in 7 of 8 prompts, "max-alive invariant at 50–55 across
  checkpoints while turnover rises" is what a repeat-structure count predicts,
  which competes with Lemma C.1's formula (`math-10.md` §5.4). `repeated_tokens`
  reaches the same ~50 without it. **Not tested directly:** the 50–55 was
  `max_alive` on the pilot's 27 checkpoints; this reads per-layer counts on
  Stage 0's 19. The direct test is `max_alive` against repeated types per
  prompt, and what sets `repeated_tokens`' deep-layer count.
