...
- **Results:**
  ...
  - The HDBSCAN partition is not reproducible run to run (ARI 5th percentile 0.347, almost all from one prompt, `repeated_tokens`: `p1d_cluster_ensemble/status-1d.md` "Float-noise drift"): it moves when activations differ by ~2e-7, one float32 rounding step. On `repeated_tokens` the moves are splits inside a near-point cloud (pairs closer than 1e-4), and average linkage moves the same way at matched k while k-means and Ward do not (`p1d_cluster_ensemble/status-1d.md` "Matched k on `repeated_tokens`"). Today's pipeline repeats itself bit for bit only because it is deterministic on one machine. A0 and F0 hold on the second sweep; per-layer claims stay exposed — `p10_cluster_function/status-10.md` §3, `p10_cluster_function/status-10.md` §3.1, `p10_cluster_function/status-10.md` §1.11
...
- **Open threads:**
  ...
  - Does the pilot's 27-checkpoint "50–55" `max_alive` also fall at layer 0? And what sets `repeated_tokens`' ~50 deep-layer clusters? (At steps 32 / 512, L6–18, the prompt is nearly one point, median pairwise cosine distance < 1e-3, so those clusters split a collapsed cloud; the trained model is not settled) — `p10_cluster_function/status-10.md` §1.8, `p1d_cluster_ensemble/status-1d.md` "Matched k on `repeated_tokens`"
...
- **Reviewed:** 2026-09-25 · body `1822cfd818`
<!-- /phase-card -->
...
- 2026-09-25 · `repeated_tokens` at steps 32 and 512 is nearly one point (median pairwise cosine distance 2e-4 to 8e-4 at L6/12/18), and shipped HDBSCAN's 34–62 clusters there are splits inside that cloud; §3's drift on it sits in pairs closer than 1e-4. Any row here that counts clusters on `repeated_tokens` at early steps counts no structure the theory names · `p1d_cluster_ensemble/status-1d.md` "Matched k on `repeated_tokens`"
