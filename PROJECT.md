...
### Resume here (2026-09-17 — the stack is merged, `main` is the only branch; the e-value audit is the live thread, Phase 1 done, Phase 1c next)

**Read this block, then §3.38, then §3.36, then §3.35, then §3.34, then §3.33, then §3.32, then
...
now past diminishing returns. §3.36 opened the **e-value audit** — the
finding that there are no e-values yet, the registry's evidence fields,
and the Phase 1 pass. §3.38 is the review-corrections pass after the
stack merged, including the substantive one: **`P-I5`'s statistic is now
intersection-union**, because min-rank had an uncontrolled Type-I rate
on the falsifier's configuration. §3.28 is the pre-registration scan — it
reclassifies §3.22's novelty and blocks §3.27's registration, so reading
§3.20–§3.27 without it will overstate what is new. Everything below this
block is earlier and is kept as background, not as the current state.

**Git state.** PRs #36–#52 are all merged; every `claude/*` branch was
deleted on 2026-09-17 and **`main` is the only branch**, at the merge of
#52 plus whatever has merged since. **PR #53** (`claude/coderabbit-p-i5-
isometric`, §3.38) is the one open PR — merge it, delete its branch. The
only untracked path is `data/hf/` (the HF cache — never `git add -A`
under `data/`). Gate green on #53's tip: **2368 passed / 5 skipped / 45
deselected**, and — new — the pure tier also passes under the **system**
interpreter, which is what CI runs; CI on `main` had been red since #51
until §3.38's fix, so check `gh run list --branch main --limit 1` after
merging. Convention from a two-session collision on 2026-09-17: **do side
work in a `git worktree`** under the scratchpad, never `git checkout -b`
in the main tree, and re-run `git status`/`git log -1` right before every
commit. The user merges PRs from GitHub (Claude Code cannot); CodeRabbit
reviews need the "🔍 Trigger review" checkbox ticked by hand. The `dead/*`
local tags hold two deleted exploratory branches (`particle-methods-
comparison`, `visualize-mets-results`) and are not pushed.

**The e-value audit — where it stands.** `claims/EXPERIMENTS.md` is the
per-phase view (phase → prediction → gate → calibration → real run);
`claims/EVALUABILITY.md` the per-prediction one; `claims/EVALUABILITY_LOG.md`
the construction diary. 39 predictions: 12 gates built, 12 calibrated, 1
run on real artifacts (`P-I5`, exploratory), **0 adjudicated**. Phase 1
is audited (§3.36): `CLAIM-C`'s gate refuses on the tree because
`gpt2-large` and `pythia-1.4b` have never been run, so the hard stop was
bypassed de facto; `CLAIM-A` has no null and its 410m step-0/8 reading
has been seen. **Two decisions are pending before Phase 1c** — spend
~6–8 h producing `CLAIM-C`'s five arms (two HF downloads, offline mode
currently forbids), or record the gate as unrun; build `CLAIM-A`'s
intersection-union null now, or leave it `needs-null`. **Phase 1c is
the next unit** (`P-gamma1`, `P-gamma2`, `P-H1`, `P-S1`; nothing has run
against Pythia there). The 16 `lit-N.md` reviews (§3.37,
`docs/LITERATURE.md`) are merged and should be read before any phase's
design is touched.
...
**Next unit: Phase 1c** (`P-gamma1`, `P-gamma2`, `P-H1`, `P-S1`) once the
two decisions above are taken.
