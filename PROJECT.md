...
`git pull --ff-only && git worktree remove ../<worktree>`. Housekeeping still
owed on GitHub: delete the merged remote branch
`claude/coderabbit-p-i5-isometric`. Only `data/` is untracked (the HF cache and
run directories — never `git add -A` under it).

**Compute is DONE and nothing is running.** All four required `CLAIM-C` arms
exist as of 2026-09-19: `pythia-1.4b-step143000` in 34 min
(`data/phase12/2026-09-19_09-44-25`) and `pythia-1.4b-random` in 17 min
(`2026-09-19_10-18-45`), against a 2 h-per-arm estimate. The `-random` arm
re-initialised on load (96 matrices, 1 embedding, checksum 173425.590 →
100442.844), which is the check `gpt2-large-random`'s old defect would have
failed. Only the optional `step0` sensitivity arm is unproduced.

**And the gate still refuses — on a metric, not an arm. Read this before
spending anything else on `CLAIM-C`.** With all four arms in,
`tools.score_claim_c` returns `REFUSED: no prompt has all six metrics in all
four arms`; verdict `INSUFFICIENT`, `hard_stop: True`, `falsified: False`, all
eight prompts dropped for `metric 'cluster_count' unavailable in at least one
arm`. **`cluster_count` is 0/9 prompts in every arm, including the two from
2026-09-17**: it comes from `clustering.json`'s `hdbscan.n_clusters`, and
`p1_mstate_tracking/clustering.py:33-38` skips HDBSCAN whenever the standalone
`hdbscan` package will not import — it is not installed, and is not named in
`requirements/`, so no run in `data/phase12` has ever had it. Worse, the writer
emits the block anyway as `{"n_clusters": null, "noise_count": 0,
"noise_fraction": 0.0}`, so `cluster_membership = 1 - noise_fraction` reads as
*available* and is **exactly 1.0 everywhere** — the gate's availability test
cannot tell "measured and flat" from "never computed". Two of six registered
metrics are dead and only one of them says so. **Decision for the author**
(detail in `status-1.md`, "The arms are all here and the gate still refuses"):
`scikit-learn` 1.9 already ships `sklearn.cluster.HDBSCAN`, so nothing needs
downloading, but swapping implementations changes the metric's provenance and
the registered statement names the metric set — and either way **all four arms
must be re-run**, ~2.5 h at the measured costs. The alternative is a registry
amendment to a smaller metric set, which inherits the constant-metric problem
unless `cluster_membership` goes too.
...
- `data/hf`: pythia-410m and -70m as before; **new:** `gpt2-large` (3.1 GB),
  `EleutherAI/pythia-1.4b` at revisions `step143000`, `step0` **and `main`**
  (11 GB, fp32 shards). All remaining `CLAIM-C` arms run with
  `HF_HUB_OFFLINE=1`.
...
- `data/phase12/2026-09-19_09-44-25`: the `pythia-1.4b-step143000` arm.
- `data/phase12/2026-09-19_10-18-45`: the `pythia-1.4b-random` arm.
...
| phase | state |
|---|---|
| 1 (`CLAIM-A`, `CLAIM-C`) | **audited** (2026-09-17). `CLAIM-A` stays `needs-null` behind `CLAIM-C`. `CLAIM-C`: **all four required arms produced 2026-09-19 and the gate still refuses** — `cluster_count` is dead in every arm because HDBSCAN has never run here. One decision open, above |
...
| 2 / 2d (`CLAIM-B`, `P-T1`, `P-M1`) | **NEXT.** Built and calibrated, unrun; `P-T1`/`P-M1` share an instrument under one claim, which their product would not show. Same method as 1c: read the status file, the registry entries and their `EXPERIMENTS.md` rows, check what the gates can actually be fed, write findings into the status file and here |
| 5b / 6 | 19 dormant, `P6-R2`/`R4` rebuilt and unrun |
| 7 | four rows built and calibrated; `P-I1` scored but unrecorded; `P-I5` parked |

**Compute to schedule — nothing, until the HDBSCAN decision above is taken.**
Re-running `CLAIM-C`'s four arms costs ~2.5 h and is wasted until then, and the
optional `step0` sensitivity arm is in the same position. One arm per
`run_1.py` invocation, each in its own run dir, never stopped mid-arm (the
scorer takes an arm whole from one directory and `run_1.py` cannot resume);
commands in `p1_mstate_tracking/status-1.md` "Remaining". After each arm, run
`tools.score_claim_c` with every run dir so far — the refusal record is the
ledger of what exists — and commit the record. `real_run_record` in the
registry gets set only when the gate emits a p-value, and the reading of
Phases 2 and 7 depends on that verdict (§3.36).
