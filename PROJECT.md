...
**Running as this was written:** `pythia-1.4b-step143000`, arm 3 of
`CLAIM-C`'s five, into `data/phase12/2026-09-19_09-44-25`, log
`data/phase12/claim_c_logs/pythia-1.4b-step143000.log`. Started 09:44, ~5 min
per long prompt, 9 prompts — faster than the 2 h estimate. **Check it finished
before doing anything else** (`Done. Results in:` is the last line); if the
memory watchdog killed it, the arm is lost whole and must be re-run, which is
why arms are one per invocation.
...
| phase | state |
|---|---|
| 1 (`CLAIM-A`, `CLAIM-C`) | **audited; both decisions taken** (2026-09-17, user). `CLAIM-A` stays `needs-null` behind `CLAIM-C`. `CLAIM-C`: arms 1–2 produced, arm 3 running; gate refuses until 4 exist |
...
**Compute to schedule, independent of the reading work — `CLAIM-C`'s two
remaining pythia-1.4b arms** (`pythia-1.4b-random`, then the `step0`
sensitivity arm; `step143000` was arm 3 and is running or done). One per
