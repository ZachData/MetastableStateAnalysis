...
| phase | state |
|---|---|
| 1 (`CLAIM-A`, `CLAIM-C`) | **audited** (2026-09-17), and **the gate has now RUN on real checkpoints — `INSUFFICIENT`, no p-value, concordance 47.9% (§3.41)**; `real_run_record` set. `CLAIM-A` stays `needs-null` behind it. Only more prompts, chosen blind, could change the verdict |
...
**Compute to schedule — nothing.** `CLAIM-C`'s four arms are done and scored;
the only unproduced arm is the optional `pythia-1.4b-step0` sensitivity one,
which cannot change the verdict (it is not in the gate's required list and the
refusal is about informative rows, not arms). Extending the prompt battery is
the one thing that could move the floor, and it is a decision (§3.41), not a
job to queue.
