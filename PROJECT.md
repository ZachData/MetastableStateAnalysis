...
### Resume here (2026-09-19 — Phase 1c audited; `CLAIM-C`'s third arm running; two PRs open and one branch unmerged)
...
**Compute, as this was written.** Arm 3, `pythia-1.4b-step143000`, is **done**
— `data/phase12/2026-09-19_09-44-25`, 9 prompts, **34 min** (09:44→10:18), so
the 2 h-per-arm estimate was high. `claims/audits/claim_c_real_run.json` (on
`claude/claim-c-arms`) carries three arms and 72 hashed files and now refuses
on **`pythia-1.4b-random` alone**. **Arm 4, `pythia-1.4b-random`, is RUNNING**
into its own run directory, log
`data/phase12/claim_c_logs/pythia-1.4b-random.log`; it re-initialised on load
(96 matrices, 1 embedding, checksum 173425.590 → 100442.844), which is the
check `gpt2-large-random`'s defect would have failed. **Confirm it finished
before doing anything else** — `Done. Results in:` is the last line — then run
`tools.score_claim_c` over all four run dirs and commit the record. If the
memory watchdog killed it the arm is lost whole and must be re-run, which is
why arms are one per invocation. After that only the optional `step0`
sensitivity arm is left, and the gate produces the project's first e-value.
...
| phase | state |
|---|---|
| 1 (`CLAIM-A`, `CLAIM-C`) | **audited; both decisions taken** (2026-09-17, user). `CLAIM-A` stays `needs-null` behind `CLAIM-C`. `CLAIM-C`: arms 1–3 produced, arm 4 running; the gate refuses on `pythia-1.4b-random` alone |
...
**Compute to schedule, independent of the reading work — `CLAIM-C`'s last
arms** (`pythia-1.4b-random` is running; the `step0` sensitivity arm is
optional and is the only one left after it). One per
...
Exact commands: `p1_mstate_tracking/status-1.md` "Remaining". After each arm,
run `tools.score_claim_c` with every run dir so far — the refusal record is
the ledger of which arms exist — and commit the record. When all four required
arms are in, the gate produces the project's first e-value; `real_run_record`
in the registry gets set then, and the reading of Phases 2 and 7 depends on
the verdict (§3.36).
