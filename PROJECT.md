...
scored but unrecorded, `P-I5` parked. **The audit's next unit is Phase 1**:
decide whether `CLAIM-C` is run on the sweep on disk (its row requires ≥ 23 %
dissenting cells and ≥ 5 usable prompts, which no existing sweep has been
checked against) and whether `CLAIM-A` gets its joint null or stays a
threshold comparison. `claims/registry.json`'s frozen fields are untouched.
