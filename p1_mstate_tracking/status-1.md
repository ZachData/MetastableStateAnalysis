...
- **Superseded / wrong:**
  ...
  - Early cluster counts track the prompt's repeat structure: at `min_cluster_size=2`, every token with 3–5 copies is clustered at step 0 — `p10_cluster_function/status-10.md` §1.7
...
- **Reviewed:** 2026-09-24 · body `5c1c65d960`
<!-- /phase-card -->
...
- 2026-09-24 · HDBSCAN (`min_cluster_size=2`) clusters copies of one token at init: at step 0 every token with 3–5 copies in the prompt is clustered and a unique one 0.18 of the time, so early cluster counts track the prompt's repeat structure · `p10_cluster_function/status-10.md` §1.7
