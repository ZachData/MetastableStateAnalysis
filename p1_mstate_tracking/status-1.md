...
- **Superseded / wrong:**
  ...
  - The carrying-capacity invariant has a competing reading as a repeat count: at layer 0 the cluster count equals the prompt's repeated token types, which training cannot change; `repeated_tokens` argues against it, so it is open — `p10_cluster_function/status-10.md` §1.8
...
- **Depends on:** none
- **Feeds:** 1b, 1c, 2, 2b, 2d, 3, 4, 5, 5b, 5c, 6, 6-frozen, 7, 10
- **Open threads:**
  ...
  - Is the carrying capacity a repeat count? `max_alive` against repeated types per prompt — `p10_cluster_function/handoff-10.md` "Parked"
- **After Phase 10:**
...
- **Reviewed:** 2026-09-24 · body `f6ac2fd086`
<!-- /phase-card -->
...
- 2026-09-24 · at layer 0 the HDBSCAN cluster count equals the prompt's number of repeated token types at every checkpoint (7 of 8 v1 prompts), and that number cannot change with training, so the carrying-capacity invariant (max-alive 50–55) has a competing reading as a repeat count. `repeated_tokens` reaches ~50 deep-layer clusters without repeats, so it is not settled · `p10_cluster_function/status-10.md` §1.8
