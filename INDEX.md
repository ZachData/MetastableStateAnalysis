...
- **READ `p10_cluster_function/lit-10.md` §11 BEFORE building on Phase 9 or
  ...
  2026-09-20. **No measured number changes.** Three constructions do: the
  parking nuclei are a **geometric** object (`δ`-separation from every preceding
  token) so **F0 measured a proxy and its failure is not evidence against the
  parking account**; the count law is **exact, distribution-free and
  dimension-free** (`E[#strong centres] = E_{x∼μ}[1/μ(B_δ(x))]`), so the
  `d ≫ 1` objection applies to the asymptotic only; and the masked system **is**
  a gradient flow — a **sequential** one, `φ̇_k = −(1/Z_k)∂E_k/∂φ_k` — so the
  Hessian hazard resolves in the project's favour and `1/Z_k` turns out to be
  F12's measurement. **Eight ladder rows added (F13–F20)**, and the next free
  row is now **F13, the centre scan**, which needs no HDBSCAN partition.
...
- **Before quoting the attention flip, read `p10_cluster_function/attention-10.md` and `math-10.md` §1 (`PROJECT.md` §3.49–§3.50).** Content-free, the causal mask already gives `received(j) = H_n − H_j` with layer mean exactly 1 — **6.155× at position 0 and 0.0038× at the last token, a ~1 600× tilt**, and the baseline hits 1.6× at position ≈53 and 0.5× at ≈160. **The observed flip is reproducible with zero content until that is divided out.** **Row A0 HAS NOW RUN and the gate has cleared** (`status-10.md` §1.1): over 3 646 layer-units the raw 0.592 gap becomes **0.036** corrected — 94 % of the flip is the mask, **all of it at initialisation**, with a learned residual only from step ~2000. Replicated on a second sweep with an independent partition. **A1–A8 are now unblocked.**
- **`lit-1.md` §4 item 1's Rényi-parking description was wrong and is corrected.** The law is `Θ(β^((d−1)/2))` — in **β and dimension, not in `n`** — and 0.7476 does not appear in it. What replaces it is better: a **free, position-indexed anchor test** (early tokens are the nuclei) and a **slope regression invariant to β's undecided unit convention** returning `d_eff = 2a+1` (`math-10.md` §5).
- **Phase 10 is the current front, and FOUR of its rows have RUN — start at `p10_cluster_function/status-10.md` (`PROJECT.md` §3.51).** F0, F1, F11-A0 and F12, over the whole 19-checkpoint 410m sweep, all **tier 1, exploratory, unregistered**. Four results: the attention flip is **~94 % causal mask**; **F0's anchor test fails in the direction it was predicted to succeed** (nuclei late, not early, under both nulls); the **identity coupling is exactly optimal in 99.5 %** of layer boundaries, so every displacement number on record is true `W_2`; and F1 + F12 together read **parked rather than pinned, in a window at steps 32–512** rather than as a property of the trained model. **Two blockers were cleared to get there**: the sweep had NO density partition (`hdbscan_labels.json` empty in 152/152) and the partition turns out **not to be reproducible run to run** — §3 of `status-10.md`. **`status-10.md` §5.1 is the ordered next-step list as REVISED on 2026-09-20 (`PROJECT.md` §3.52.4): F13 (the centre scan) and F14 (observed count vs Lemma C.1) now sit ahead of the `CLAIM-C` floor comparison and of A1–A8, because they need no partition at all.** Note also that **F0's result is not evidence against the parking account** — it measured a proxy for the paper's object (`lit-10.md` §11.4).
- **Phase 9 is PARKED on Phase 10, not closed.** `p9_metric_intervention/notes-9.md` then `plan-9.md`; `notes-10.md` §9 states what each phase owes the other. Amendments marked inline in `plan-9.md`: §2.1a, **§5.1a (RESOLVED 2026-09-20 — the gradient-flow hazard is smaller and more specific than it read)** and **§5.1b (new — the transfer-operator construction exists, and its reversibility assumption must not be copied)**. `notes-9.md` §8 carries two corrections from the same read: the Hessian claim is per-token, and **two thirds of the novelty differentia fail** — what survives is **state versus operator**. **Any forgetting result now needs a relearning arm.**
- **New convention: `docs/readings/<arxiv-id>.md`** for a paper read as primary text, marked **[R]**. First entry is **`docs/readings/2411.04990.md`**; Phase 10's own read of the same paper is `lit-10.md` §11–§15 and `math-10.md` §7, and **the two agree except on the gradient-flow question, where `lit-10.md` §11.5 is the later and more complete reading** (Lemma 5.3: a *sequential* gradient flow).
- **Four consequences of `2411.04990` that are not in the Phase 10 bullets below.** **Thm 4.1: all tokens converge to `x₁(0)`, the first token's initial position, for arbitrary `Q, K`** — so no QK-side intervention, γ included, prevents collapse under `V = I_d`, and `plan-9.md` §4.3's ceiling is understated. **Lemma C.1's centre count saturates in `n`** — that is Phase 1's carrying-capacity finding (50–55 invariant) with a formula. **The `d_eff` regression is the paper's own open conjecture**, with `d₁ = dim L` computable from Phase 2's projectors, which makes the test differential. **RMSNorm's diagonal is absorbable into `K, Q, V`**, so a γ-patch is *not* a read-side lever and `plan-9.md` §4.7's sign test needs a `W_V` arm.
