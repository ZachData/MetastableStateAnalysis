...
- **Before quoting the attention flip, read `p10_cluster_function/attention-10.md` (`PROJECT.md` §3.49).** Position 0 is the sink and is unclustered by construction; the causal mask gives early tokens a `1/j` advantage the statistic does not divide out; it is a mean summed over all heads; and it has never run on Pythia. **Row A0 of its ladder gates the rest.**
...
| 10 | `p10_cluster_function/` | **Notes, an audit and a partial scan. Pre-design, nothing frozen, nothing registered** — *what clusters are and what they do*, split out of Phase 9 (`PROJECT.md` §3.48–§3.49). `notes-10.md`: the trash-collection hypothesis as a four-signature concordance, the Rényi-parking cluster count, the **Jacobian lens**. `attention-10.md`: the attention flip audited — four unchecked confounds, two structural — plus the paid/received 2×2 and **`Z_beta,i`**. `lit-10.md` is a **partial** trigger-1 scan. No code yet |
