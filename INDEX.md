...
| 10 | `p10_cluster_function/` | **Notes and a partial scan. Pre-design, nothing frozen, nothing registered** — *what clusters are and what they do*, split out of Phase 9 (`PROJECT.md` §3.48). Read `notes-10.md`: the trash-collection hypothesis as a four-signature concordance, the Rényi-parking cluster count as its quantitative arm, and the **Jacobian lens** as the instrument that makes the functional partition measurable per layer. `lit-10.md` is a **partial** trigger-1 scan — one question settled, the rest named in `notes-10.md` §12. Carries a hazard for Phase 9 (§10.1) and three distinctions the project had not drawn (§5–§7). No code yet |
