...
Cost tags follow `archive/UPDATE_PLAN.md`: **[D]** doc-only · **[R]** report-only, data already on
...
e-values valid (`archive/POPPER_PLAN-done.md` §B3) is a CI check or it is nothing, and the bridge's whole point is to emit
...
| Baseline suite health | **zero tests collected** before this pass; see `archive/docs/CI_BASELINE.md` |
...
pre-registration by dated addendum rather than edit; `archive/UPDATE_PLAN.md` §6 lists six standing
...
`archive/UPDATE_PLAN.md` §5.6 (a wrong trace contraction that the identity-case anchor could not
...
**A0–A7: DONE, archived 2026-09-22** to `archive/POPPER_PLAN-done.md` (verbatim).
...
**B1–B5, B7: DONE, archived 2026-09-22** to `archive/POPPER_PLAN-done.md` (verbatim).
...
**C1, C2: DONE, archived 2026-09-22** to `archive/POPPER_PLAN-done.md` (verbatim).
...
1. **The suite collected zero tests**, not "most of them" — a conftest-level
   ...
   so those phases' tests had never run once. See `archive/docs/CI_BASELINE.md`.
...
- **Multi-degree combination.** One pre-declared scalar -- the sum over degrees
  ...
  seeing three p-values. Standardising is load-bearing: `archive/UPDATE_PLAN.md` §5.8
...
$U_\text{neg}$ and $U_A$ — as a live alternative explanation, and `archive/p6_subspace/design-6.md`
...
`archive/docs/CI_BASELINE.md` records it, because a tool that used to be worth running
...
question a reader arrives with and no file here could answer — **which phase,
which experiment, carries an e-value, and what is it worth?** — and then fixed
what asking it turned up. Five findings: four are joins between the pieces, and
the fifth (§5 below) is about a null.

The arithmetic is not among them. `core/evalues.py`'s calibrator, log-space
product, decision rule and `required_p_for_rejection` are correct as written, and
that was checked before anything else.

**The answer was spread across five files and the join existed in none of them.**
...
knows what has been adjudicated; this plan's §6a–6w knows what each construction
cost; each phase's `status-N.md` knows what the phase measured. The registry had
no `phase` field at all, so the phase→prediction join lived inside the
`instrument` field's prose (`"1c-F (p1c_frames/design_test.py, centroids.py)"`)
where nothing could check it. `claims/EXPERIMENTS.md` is that join, generated by
`tools/render_experiments.py` and `--check`ed in tier 0 like the other three.

Three descriptive fields carry it: `phase` (the directory the instrument lives
in, as `INDEX.md` names phases), `experiment` (the instrument's own id where it
names one) and `gate` (the `module:function` computing the p-value, `""` where no
null is built). They are **not** frozen fields — they describe where an
instrument lives, not what a prediction claims, and freezing them would make a
directory rename read as an amendment.

### 1. The map's evidence column was wrong in the flattering direction first

The obvious way to link a dry run to the prediction it covers is to scan the
record for the prediction's id. It over-credits, and it over-credits in exactly
the direction that would have hidden this pass's main finding.
`claims/audits/p_t1_p_m1_dry_run.json` cites `CLAIM-B`, `CLAIM-C`, `P6-R2`,
`P6-R4` and `P-S1` in its prose — cross-references to what earlier passes found,
not coverage — so the id scan credited six predictions with a dry run that
measured two.

What these records state exactly is which **file** they were measured against:
every one records its gate's path and SHA-256, which is how §6h's hash discipline
already works. So the link is the gate module, reached directly or one hop
through the `tools/…` script the record names as its generator — the hop is
needed because a calibration typically records the tool rather than the gate the
tool imports. `tests/test_tools_render_experiments.py` asserts the distinction on
whatever the audits currently cite, rather than on a list that would go stale the
next time a record is regenerated.

### 2. Two adjudicable gates never had a known-answer dry run

With the link computed correctly, nine of the eleven adjudicable rows have one
and **`P-AB1` and `P-I3` do not**. Both were built after §6p closed
`EVALUABILITY.md`'s queue — §6q and §6s — in the floor-first order that pass
recommended, and both carry a calibration
(`claims/calibration/{patching_exponent,cross_head_association}.json`) whose
`validity` block measures the H0 rejection rate conditional on emission. So this
is not an absence of evidence; it is an absence of *that* evidence, and the two
questions are different. A calibration measures how the construction behaves on
a synthetic family. A dry run asks whether the gate returns the verdict that is
correct a priori.

§6p's own base rate is the argument for caring: *"Nine for nine, every one of
them changed something … not one converted row survived being run on an input
whose answer was already known, and no test was failing on any of them."* Two
rows have skipped the step with the highest recorded yield in this project, and
until this pass nothing said so.

### 3. Three active phases carry no falsifier at all

`INDEX.md` names 7d, 7e and 8 as the active work. None has a registered
prediction, so none can reach `core/adjudication.py` — which is not a bug in
those phases and not something the apparatus could have reported, because the
apparatus can only refuse what was registered. `tools/check_registry.py`'s
coverage scan runs the other way: ids in the tree must be in the registry, never
phases in the tree must have ids.

What those phases do have is *measured control distributions* — 7d's
`coverage_by_others_null` and near-median control heads, 8's "heads clearing the
rung's OWN control distribution". That is the shape §6e described before the
first null was constructed: a comparison against a reference that reads like
significance without being calibrated as one. `status-8.md`'s "invariant 5 does
not replicate" is a threshold-free cross-rung statement and correctly makes no
p-value claim; the point here is only that nothing downstream can tell that from
a claim that was adjudicated, and now `claims/EXPERIMENTS.md` says which is which.

Recorded, not fixed. Registering a Phase 8 prediction is a scientific decision
and the rung policy in `p8_scale_ladder/design-8.md` already reserves 1b and 1.4b
for exactly that purpose.

### 4. `H-BUDGET` is declared and nothing names it

`claims/CLAIMS.md` §304 declares six claims. Five have predictions.
`H-BUDGET` — *the network spends a bounded dimensionality budget on particles
that must stay individuated* — has none, so its e-process has no factor that
could ever enter. `check_registry.py` already failed a prediction naming a claim
that is not a `CLAIMS.md` heading; it now warns the other way too, because an
E of 1 from no factors reads identically in `FALSIFICATION.md` to an E of 1 from
factors that did not cross.

### 5. `P-M1`'s refusal conditions on its own statistic, and its audit shows the shape

The one finding here that is about a *null* rather than about a join, and the
only one this pass flags without fixing.

`p_value_p_m1` refuses when the mean/min/max head-to-layer aggregates disagree in
sign — a good instinct, and §6p's reason for it is right: emitting a p-value for
one arbitrarily chosen aggregate would turn a resolution limit into a result.
What makes it different from `P-T1`'s refusal, which it resembles, is **what it
conditions on**.

`P-T1` refuses on the 2×2 table's margins. The permutation null holds those
fixed, so they are ancillary, conditioning on them costs nothing, and the emitted
p stays valid conditional on emission. `P-M1` refuses on the observed sign
pattern of three correlations — and the **mean** aggregate's correlation *is the
test statistic*. Conditioning on "all three agree in sign" selects on |corr|, and
under a null symmetric in sign that selection runs in the anticonservative
direction for a one-sided test.

**The rate that governs the ledger is the one conditional on emission** — §6g's
point, and `claims/audits/p_t1_p_m1_dry_run.json` measures exactly it. Ordered by
emission rate, P-M1's six emitting designs read:

| emission rate | H0 rejection | design |
|---|---|---|
| 0.940 | **0.0638** | 12 layers, 2 violations |
| 0.975 | **0.0718** | 24 layers, 2 violations |
| 0.975 | **0.0615** | 36 layers, 2 violations |
| 0.990 | **0.0505** | 49 layers, 1 violation |
| 1.000 | 0.0200 | 12 layers, 3 violations |
| 1.000 | 0.0350 | 24 layers, 1 violation |

**Every design where the refusal fired at all sits at or above the nominal 0.05;
both designs where it never fired sit well below.** `P-T1`'s seven designs, whose
refusal is the ancillary one, are all below 0.05 and show no such ordering.

This is a flag, not a verdict, and the reason is in the numbers: at 200
replicates per cell the standard error at α is 0.0155, so no single row is
significant and the audit's own 3.17-SE bound passes honestly. Four of six above
nominal with the two cleanest cells the two lowest is a pattern, not a
measurement. What would settle it is cheap and has not been run — replicates at a
cell with a **low** emission rate, where the selection is strongest and the
present grid is thinnest, since every cell measured emits at 0.94 or above.

Recorded in the gate itself as well as here, because a reader of
`gradient_flow_condition.py` is the one who needs it. Not changed: what to do
about a registered gate's refusal rule is a scientific decision, and the two
obvious repairs are not equivalent — dropping the refusal emits a number the
design cannot support, while pre-declaring the mean aggregate alone gives up the
resolution check that the refusal exists to enforce.

### What the pass changed in the gates, and the one thing it deliberately did not

**`p6_subspace/r2_r4_null.py` took `alpha: float = 0.05` as a default argument**
on both `p_value_p6_r2` and `p_value_p6_r4`, four lines below a comment
describing that same number as "two orders below **the registry's alpha**". Every
other gate that needs an alpha resolves it from `claims/registry.json` at call
time; this one had a literal. A registry edited from 0.05 to 0.01 would have
moved eight gate modules' refusal boundaries and left this one at the old value,
with every generated table still printing 0.01 at the top. Now resolved at call time,
with an explicit `alpha=` still winning.

(`p1c_frames/centroids.py` has no alpha and correctly needs none: `P-S1` is the
one adjudicable row with no three-way verdict lattice, so it has no reciprocal
branch and nothing for an alpha to decide. Its `adjudicate_p_s1_banded` effect-
size floor is a reporting object beside the p-value, not a cut on it.)

**The six duplicate resolvers were left in place**, and the reason is the
apparatus working rather than an oversight. `core.adjudication.registry_alpha`
is now the declared copy, but collapsing them into it means editing six gate
files whose bytes are pinned by committed dry runs — and
`claims/audits/p_i1_attainable_floor.json` pins `core/changepoint_colocation.py`
and cannot be regenerated without `data/analysis/formation_series.json`, which is
git-ignored bulk. The tidy-up was written, cost an evidence record that cannot be
rebuilt here, and was reverted. Its docstring records where the six copies are so
the next pass that regenerates those records for a scientific reason can take it
for free.

Two stale references fixed on the way: `core/evalues.py` cited
`tools/check_preregistration.py`, which has never existed (the function lives in
`tools/check_registry.py`), and `tools/calibrate_steering_sign.py` wrote
`"ALPHA_SPREAD_FRACTION = 0.1"` into the artifact that is the *evidence for that
constant* while the shipped value was 0.2 and the same file's `constants` block
reported 0.2 correctly — so the record disagreed with itself about which number
it had measured. It now reads the module.

### What this pass did not find

Nothing wrong with the arithmetic. `core/evalues.py`'s calibrator, log-space
product, decision rule and `required_p_for_rejection` are correct as written;
every gate routes through `adjudicate_if_registered` and every one threads
`adjudications_dir`, so no test path can reach the real ledger; every audit and
calibration record's committed SHA-256 matched its file on disk before this pass
touched anything. The defects above are all in the *joins between* the pieces,
which is consistent with §6p's own taxonomy — three of its four defect kinds are
checkable before any data exists, and so were these four.
...
- It does not run any science. No chunk here adjudicates a prediction; B6 makes adjudication
  *possible*, and the first real adjudication is a separate decision gated on
  `tools/preflight_1c.py` per `archive/UPDATE_PLAN.md` §3.
