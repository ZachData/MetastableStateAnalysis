...
def render_by_phase(reg: dict) -> str:
    """The per-phase view lives in claims/EXPERIMENTS.md; this section points there."""
    n_phases = len({p.get("phase") for p in reg.get("predictions", [])})
    return (
        f"The per-phase view — phase → experiment → prediction → gate, with each "
        f"row's calibration and real-run evidence — is `claims/EXPERIMENTS.md`, "
        f"generated by `tools/render_experiments.py` from the same registry "
        f"({n_phases} phases carry registered predictions; the rest are exploratory "
        f"by design and nothing in them may carry an e-value). This file is the "
        f"per-prediction view; the two are generated from one source and neither "
        f"is edited by hand.\n"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    updated = _replace_section(updated, "## By phase", render_by_phase(reg))
