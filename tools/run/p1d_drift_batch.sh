...
# tools/run/p1d_drift.py. Usage: p1d_drift_batch.sh <out_root> [step ...]
set -euo pipefail
MAIN=/run/media/system/WDS_500/Mets
...
export CUDA_VISIBLE_DEVICES="" METS_REPO=$PWD METS_DATA=$MAIN/data
for step in $STEPS; do for key in $KEYS; do
  s0=$($PY -c "import json;print(json.load(open('$MAIN/data/phase12/stage0_logs/stage0_index.json'))['runs']['$step|$key'])")
...
for sk in "143000 wiki_paragraph" "32 camus_letranger"; do
  set -- $sk
  s0=$($PY -c "import json;print(json.load(open('$MAIN/data/phase12/stage0_logs/stage0_index.json'))['runs']['$1|$2'])")
