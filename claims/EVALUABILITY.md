...
| `P-AB1` | H-BRIDGE | **e-value** | 0.8 | Paired growth-exponent comparison against a MATCHED RANDOM-DIRECTION ablation of EQUAL MAGNITUDE at the same layer -- the same control design design-5c.md already requires. The control is not optional, and the registry's original reason (later layers have more opportunity to diverge) is right but not the binding one: a fitted exponent has NO window-free meaning, because a saturating curve's log-log slope falls with the fit window and the ablation point fixes that window. Both arms are therefore fitted over one COMMON window and compared only within an ablation point. The registered 'permutation over ablation points' is NOT the null used: read literally it permutes the pairing, which leaves a mean paired difference invariant and floors the design at p = 1.000. The null is the exact SIGN FLIP of the two arms' labels -- under H0 a real direction and a structureless one of equal magnitude at the same layer are exchangeable -- combined by a sign sum, floor (2^(n-k)+1)/(2^n+1), so six informative units is the first design that clears alpha and an ODD number of ablation points per prompt is free while an even one is not. The exchangeable unit is the PROMPT, registered by the author on 2026-08-27 after the measurement was put to them: the per-ablation-point reading, which this field's earlier wording implied, inflates to 0.235 under a shared per-prompt factor where the per-prompt reading holds at 0.029. It is a trade and not free -- it costs resolution (floor 2/(2^n_prompts+1), so six prompts) and power (0.503 against 0.856 at a planted gap of 0.15) -- and adjudication turns away a result computed under any other unit. A FIXED offset between the real and control direction populations is separated by neither unit and reaches 1.000 -- stated, diagnosed, not removed. The gate refuses when either arm's divergence is not a power law over the window, because the exponent is not monotone in propagation strength: the arm that is larger at every layer saturates sooner and reads as LESS superlinear, which returned the RECAPTURES falsification branch on 0.98-1.00 of draws where the correct verdict is INSUFFICIENT. See claims/calibration/patching_exponent.json and POPPER_PLAN.md 6q. |
...
| `P-I3` | H-BRIDGE | **e-value** | 1.0 | Correlation with a REQUIRED control arm over non-induction heads; motif_stats.py makes independence_source a positional argument so the arm cannot be omitted by default. Permutation over the head classification. -- SUPERSEDED 2026-08-30, see below. Motif-rate contrast between each induction head and CONTROL HEADS MATCHED ON ITS BEHAVIOURAL INDUCTION SCORE, built 2026-08-30 (p7_motifs/cross_head_gate.py, claims/calibration/cross_head_association.json, POPPER_PLAN.md 6s). THE REGISTERED 'permutation over the head classification' IS NOT THE NULL USED, and the reason is arithmetic rather than a matter of power: an induction head is one whose behavioural induction score clears a cutoff, so the classification is a THRESHOLD ON THE VARIABLE THE STATISTIC CORRELATES AGAINST, and exactly ONE of the C(n,k) label assignments that null draws from is a possible classification -- 1 of 1.09e16 at 384 heads and 8 induction heads. Measured on both readings of the statistic on the same draws, neither discriminates a planted effect from its absence (\|discrimination\| under 0.10 either way), and the slope reading is anti-conservative on plain H0 besides. WHAT THE STATISTIC DEGENERATES ON is the within-group spread of the behavioural score, which the classification sets: on ONE population carrying ONE relation the within-group correlations differ by a large negative contrast -- the FALSIFIER's direction -- with no interaction present, and the within-group slope removes that bias while its sampling spread runs an order of magnitude above the control arm's. Neither within-group association statistic is adjudicable and both are reported. THE STATISTIC ADJUDICATED is the quantity this entry's own h0 and falsifier name -- 'unrelated to the classification', 'carry the motif at the same rate' -- a LEVEL contrast at matched score: the induction head's rank among its matched set, summed over sets. The prediction's wording names a third quantity, the within-group correlation, and one number cannot carry both questions (P-I1's gate made the same split from the other side). THE MATCH IS STRADDLED, M/2 control heads below the induction head's score and M/2 above, because a nearest-by-score match leaves a one-signed residual gap and reads curvature in the score-to-motif relation as an effect. M = 4 is DERIVED: power stops rising there while every further control drops another induction head off the top of the score ranking. THE NULL is the induction label exchangeable within a matched set, ENUMERATED in closed form -- so unlike P-AB1 there is one floor and not two, and 6p's max-of-two rule applies with its sampling term absent. Design floor 1/(M+1)^n_informative_sets, with an all-tied set enumerated out: CLAIM-C's informative rows (6l) and P-AB1's odd ablation grids (6q) reached by a third group. THE TAUTOLOGY IS CAUGHT BY THE FLOOR, which is the finding. PREDICTIONS.md's Phase 7 adjudication constraint 2 names this phase's central methodological danger and P-I1's gate could only refuse the degenerate case; here, when the classification IS the thresholded score no induction head has a control above it, no set can be straddled, and the design floor is 1.000 -- decidable before an edge is counted. The leak is proportional to how hard the motif tracks the score rather than to a literal identity, which is a larger family than the constraint's wording describes. THE EXCHANGEABLE UNIT is the HEAD, fixed by PREDICTIONS.md's Phase 7 adjudication constraint 1 rather than chosen here, and the effective n is sharper than that constraint states: it is the number of induction heads that can be MATCHED. CONTROL MATCHING KEY = 'score_and_layer', registered by the author on 2026-08-30 after the measurement was put to them: induction heads cluster in a band of layers and a shared elevation across that band is invisible to a control matched on score alone, climbing past 0.15 at one standard deviation, while a control drawn from the induction head's own layer is flat. It is a TRADE and not free -- roughly a quarter of the informative sets, a p-value emitted on well under two thirds of draws instead of all, and power roughly halved -- and adjudication turns away a result computed under the other key while p_value_p_i3 still computes under either and reports which. THE FALSIFIER as registered ('non-induction heads carry the motif at the SAME rate') describes the null and maps to INSUFFICIENT (6k's rule); the falsification branch an e-process can carry is ACTIVATION_PROPERTY -- control heads matched on score carrying MORE of the motif than the induction heads they are matched to -- and it is checked to be a branch that can fire. p_reciprocal is a stop-rule input only and enters no E. STILL NO DATA: the head tables are synthetic with planted answers, no motif sweep exists in this repository, and claims/adjudications/ is empty. |
...
| `P-I7` | H-BRIDGE | **needs-null** | 1.0 | DESIGN FIXED HERE; THE NULL ITSELF IS TO BE BUILT AND CALIBRATED IN THE CHUNK, which is why this registers as needs-null rather than claiming an e-value it does not yet have.  THE PRE-REGISTRATION CONSTRAINT THAT DEFINES THIS ENTRY. The pythia-410m artifact is SPENT: PROJECT.md 3.12-I/J measured this quantity across all 384 heads and 19 checkpoints on 2026-09-09 and the result is already known (median 0.523, 11 heads above 0.7, max 0.956, L5H2 flat at ~0.50 throughout). tools/check_registry.py rule 3 forbids adjudicating a prediction on an artifact seen before it was registered, and that is exactly the conditional-validity violation no other check catches. SO THIS ENTRY MAY ONLY BE ADJUDICATED ON A MODEL THIS PROJECT HAS NOT MEASURED -- pythia-70m under PROJECT.md 3.9's own registered grid is the intended site, and PROJECT.md 5.3 records that neither its checkpoints nor its weights are on disk yet. The 410m instance is REPORTED AS THE MOTIVATING OBSERVATION and enters no p-value, ever.  IDENTIFICATION MUST NOT READ SYMMETRY, or the test is circular. CONTENT matchers are identified by a behavioural induction score (the construction behind data/analysis/behavioural_series.json); POSITIONAL matchers by offset-attention, the property that makes L5H2 a previous-token head (mean attention at offset -1 of 0.895 against a 384-head median of 0.018). Both are activation-side and neither touches the weights the statistic is computed from.  STATISTIC: the rank of each identified head in its own model's 384-head symmetric-fraction distribution, contrasted between the two classes. Ranks rather than raw values, because absolute symmetry is not comparable across models of different width and depth and a model-wide symmetry trend is exactly the confound. ALTERNATIVE: one-sided, content above positional.  NULL: permutation of the CLASS LABEL across heads with both marginals held fixed -- P-T1's construction, reused rather than reinvented. Holding the symmetry distribution fixed is what makes a model-wide trend invisible to the null, which is the shared-model factor PROJECT.md 3.14.1 names as 7a's hardest problem. EXCHANGEABLE UNIT: the HEAD.  TO BE BUILT BEFORE THIS BECOMES e-value, and P-T1's entry is the template because it hit each of these: (1) THE ATTAINABLE FLOOR IS NOT 1/(n_perm+1). The statistic is discrete over ranks with fixed marginals, so the floor is set by the marginals the way P-T1's hypergeometric floor is, and must be computed in closed form and refused on when it exceeds alpha. (2) Calibration on planted inputs, both directions, since POPPER_PLAN.md 6h found an audit arm reporting PASS while incapable of failing. (3) A decision, taken in advance, on how many heads of each class the identification admits and what happens when a model yields too few -- pythia-70m has 6 layers x 8 heads = 48 heads against 410m's 384, and the floor may not clear alpha at that size. That is a precondition on the model, computable before it is swept, in the shape CLAIM-B's 19-control-series requirement takes.  RELATION TO PUBLISHED WORK, recorded because it is what makes this differential rather than confirmatory. Saponati et al. (arXiv 2502.10927) find that AUTOREGRESSIVE training induces DIRECTIONALITY in W_QK, reported as a median across layers with no per-head breakdown and no positional-dimension exclusion. This entry predicts the opposite for an identified subpopulation, on the RoPE-excluded content operator. Their aggregate and this tail are compatible -- 410m's median is 0.523 -- but a reader must be able to see that the prediction is against the published population trend and not a restatement of it. 'The Routing and Filtering Structure of Attention' (arXiv 2605.18826) supplies the mechanistic vocabulary: symmetric = filtering (undirected mutual relevance), skew = routing (directed transport). In that vocabulary this entry predicts content matchers become almost pure FILTERS, with routing left to RoPE and the causal mask. |
