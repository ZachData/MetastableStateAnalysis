...
## 5. The one remaining blocker — unchanged
...
correctly — re-read at HEAD this session and it still does.
...
contributes, and only then build the control.

* **Step 2 is done.** Across the 8 battery prompts at step 54000 the raw relay
  count against the prompt's own induction-pair supply runs **r = +0.9958** —
  99% of the cross-prompt variance is the prompt's combinatorics, not the
  model's circuitry. Excluding `repeated_tokens`, +0.8908. Nothing else is
  close: n_tokens −0.39, n_same_content −0.36, n_distinct_tokens −0.79.
* **Step 3 is done**, and §3 above is its answer: the grid contributed the
  entire previous failure, and the fills fixed it.
* **Step 1 (the attainable floor) and the control itself are NOT done.**

The constraint that falls out is unchanged: a relay-count null that does not
hold `n_induction` fixed per prompt is testing whether the prompt has induction
...
**§3's non-monotonicity is new input to this design.** A null built to explain
a monotone rise will not describe a series that peaks between 54000 and 143000
while spreading across 34 more heads.
...
python tools/run/curve.py   # ~2:43, writes data/analysis/curve.json
```
